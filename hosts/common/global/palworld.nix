{
  ...
  runWrapper = pkgs.writeShellScript "palworld-run-wrapper" ''
    ...
    export SERVER_PWD ADMIN_PWD

    echo "Generating PalWorldSettings.ini..."
    mkdir -p Pal/Saved/Config/LinuxServer
    ${pkgs.envsubst}/bin/envsubst '$SERVER_PWD $ADMIN_PWD' \
      < ${palWorldSettings} \
  ...
  services.steamcmd-servers = {
    ...
    servers = {
      pal = {
        ...
        preStart = ''
          echo "Linking execution wrapper..."
          ln -sf ${runWrapper} run-wrapper.sh
        '';
